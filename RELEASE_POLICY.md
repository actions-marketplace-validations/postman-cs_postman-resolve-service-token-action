...
The CLI publishes as `@postman-cse/onboarding-resolve-service-token` with versions that match the GitHub release tag. The rolling `v1` alias updates the action channel and skips npm publishing.

## Compatibility

The `v1` channel keeps action inputs and outputs compatible unless a security fix requires narrower behavior. New optional inputs can be added under `v1` when they preserve existing workflows.
