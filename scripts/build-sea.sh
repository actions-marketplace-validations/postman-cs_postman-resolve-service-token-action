...
# SEA recipe for the postman-resolve-service-token CLI (Linux x64). Built on a
# native linux-x64 CI runner (the authoritative build); assumes deps are installed
...
BIN="$OUT_DIR/postman-resolve-service-token-${VERSION}-linux-x64"
