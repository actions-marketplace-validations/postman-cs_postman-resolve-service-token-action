...
/** Rebuild dist from source for the release commit. */
export function rebuildDist() {
  run('npm', ['run', 'bundle']);
}

/**
 * Prove the committed dist matches the release contract.
 *
 * verify-dist-artifact.mjs reads the working tree, so it is only meaningful
 * AFTER the release commit exists.
 */
export function assertCommittedDistMatchesSource() {
  run('npm', ['run', 'bundle']);
  run('git', ['diff', '--exit-code', '--', 'dist']);
  run('node', ['scripts/verify-dist-artifact.mjs']);
}
