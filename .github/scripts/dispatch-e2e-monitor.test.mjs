...
test('sends exactly the four inputs the listener reads, and no event_type', async () => {
  ...
  assert.equal(body.inputs.action, 'postman-resolve-service-token-action');
...
test('failure text names the action and ref, and never leaks the token', async () => {
  await assert.rejects(
    ...
    (error) => {
      assert.match(error.message, /HTTP 422 for postman-resolve-service-token-action@v9\.9\.9/);
...
test('an unreadable error body still reports the status code', async () => {
  await assert.rejects(
    ...
    /HTTP 503 for postman-resolve-service-token-action@v9\.9\.9: <response body unavailable>/
  );
});
...
test('success is announced with the correlation id for run-name tracing', async () => {
  ...
  assert.equal(result.correlationId, 'postman-cs-postman-resolve-service-token-action-4242-2-v9.9.9');
