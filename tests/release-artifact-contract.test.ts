...
function stageReleaseDirectory(packageVersion = '2.0.4') {
  ...
  writeFileSync(
    ...
    JSON.stringify({ name: '@postman-cs/onboarding-resolve-service-token', version: packageVersion })
  );
  ...
  const manifest = {
    ...
    package_name: '@postman-cs/onboarding-resolve-service-token',
    ...
    try {
      expect(() =>
        validateManifest(manifest, directory, {
          ...
          commitSha: 'abc',
          tag: 'v2.0.4',
          packageName: '@postman-cs/onboarding-resolve-service-token',
