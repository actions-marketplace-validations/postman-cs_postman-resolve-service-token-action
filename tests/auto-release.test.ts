 ...
 *
 * Pins the invariants whose absence burned immutable release tags elsewhere:
 * a burnt version number must be skipped rather than reused, and the tag must
 * be created only after the committed release bytes pass every gate.
 */
...
describe('release version selection', () => {
  it('skips a burnt version instead of reusing it', () => {
    const plan = selectNextVersion({
      current: '2.0.4',
      bump: 'patch',
      takenTags: ['v2.0.4', 'v2.0.5']
    });
    expect(plan.version).toBe('2.0.6');
    expect(plan.skipped).toContain('2.0.5');
  });

  it('never returns a version that is already tagged', () => {
    const taken = ['v2.0.0', 'v2.0.1', 'v2.0.2', 'v2.0.3'];
  ...
  it('maps conventional commits onto semver bumps', () => {
    ...
    expect(applyBump('2.0.5', 'minor')).toBe('2.1.0');
  });

  it('does not cut a release for release-plumbing commits alone', () => {
    expect(parseConventionalBump(['chore(release): v2.0.5'])).toBeNull();
...
describe('release-cut ordering contract', () => {
  it('creates the tag only after the committed release bytes are verified', () => {
    ...
    expect(
      releaseCutSource.indexOf('assertCommittedDistMatchesSource();')
  ...
  it('rebuilds dist before committing and verifies committed dist before tagging', () => {
    ...
    expect(releaseCutSource).toContain("run('git', ['diff', '--exit-code', '--', 'dist'])");
  });

  it('stages only release paths and force-adds gitignored dist', () => {
    expect(releaseCutSource).toContain("'add', '-f', 'dist'");
    expect(releaseCutSource).toContain('release commit touched non-release paths');
