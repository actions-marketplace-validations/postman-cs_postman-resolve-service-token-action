...
function planPackedBinInvocation(
  ...
  if (platform !== "win32") {
    if (args.length !== 1 || !PACKED_BIN_FIXED_ARGS.has(args[0]!)) {
      throw new Error(
        `packed-bin args must be exactly --help or --version; got ${JSON.stringify(args)}`,
      );
    }
...
describe("packed-bin invocation plan", () => {
  it("keeps POSIX as direct executable plus args", () => {
    const packedBinPath =
      "/tmp/prefix/node_modules/.bin/postman-resolve-service-token";
    expect(
      planPackedBinInvocation(packedBinPath, ["--help"], { platform: "linux" }),
    ).toEqual({
      file: packedBinPath,
      args: ["--help"],
    });
    expect(
      planPackedBinInvocation(packedBinPath, ["--version"], {
        platform: "darwin",
      }),
    ).toEqual({
      file: packedBinPath,
  ...
  it("selects ComSpec/COMSPEC/cmd.exe on win32 and never plans execFile of .cmd", () => {
    const packedBinPath =
      "C:\\Program Files\\pkg\\postman-resolve-service-token.cmd";
    ...
    expect(viaComSpec.args).toEqual([
      "/d",
      "/s",
      "/c",
      `""${packedBinPath}" "--help""`,
    ...
    const viaComspecEnv = planPackedBinInvocation(
      packedBinPath,
      ["--version"],
      {
        platform: "win32",
    ...
    expect(viaComspecEnv.args).toEqual([
      "/d",
      "/s",
      "/c",
      `""${packedBinPath}" "--version""`,
    ]);
  ...
  it("rejects cmd.exe metacharacters in win32 path and args before invoke", () => {
    ...
    expect(() =>
      planPackedBinInvocation(safe, ["--unknown"], {
        platform: "win32",
        env: {},
      }),
    ).toThrow(/packed-bin args must be exactly/);
    expect(() =>
      planPackedBinInvocation(safe, ["--help", "--version"], {
        platform: "linux",
      }),
    ).toThrow(/packed-bin args must be exactly/);
  });
...
describe("CLI packaging contract", () => {
  ...
  it("runs ./dist/cli.cjs --help and --version without credentials, network, or writes", async () => {
    ...
    const sandbox = await makeTempDir(
      "postman-resolve-service-token-cli-sandbox-",
    );
    const env = {
  ...
  it("prefers CLI credential flags over action and plain environment values", async () => {
    const cliPath = path.join(repoRoot, "dist", "cli.cjs");
    const sandbox = await makeTempDir(
      "postman-resolve-service-token-precedence-",
    );
  ...
  it("packs, prepares, and runs postman-resolve-service-token --help/--version without side effects", async () => {
    ...
    const prefixDir = await makeTempDir(
      "postman-resolve-service-token-prefix-",
    );
    const binSandbox = await makeTempDir(
      "postman-resolve-service-token-bin-sandbox-",
    );
    const distBefore = new Map(
      await Promise.all(
        ["cli.cjs", "index.cjs"].map(
          async (name) =>
            [name, await readFile(path.join(repoRoot, "dist", name))] as const,
        ),
      ),
    );
    ...
    if (process.platform !== "win32") {
      expect(
        packed.files.find((file) => file.path === "dist/cli.cjs")?.mode,
      ).toBe(0o755);
    }

    const tarballPath = path.join(packDir, packed.filename);
    const packedManifestResult = await execFileAsync(
      "tar",
    ...
    const packedBinEntries =
      typeof packedPackageJson.bin === "string"
        ? [
            [
              packedPackageJson.name.replace(/^@[^/]+\//, ""),
  ...
  it("does not rebuild dist from packaging tests", async () => {
    ...
    const packagingSource = await readFile(
      path.join(repoRoot, "tests", "cli-packaging.test.ts"),
      "utf8",
    );
    ...
    expect(packagingSource).not.toMatch(
      new RegExp(bannedRebuild.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
    );
